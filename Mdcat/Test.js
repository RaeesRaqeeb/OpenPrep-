...
async function submitQuestionReport() {
  ...
  try {
    ...
    if (!data?.id) {
      showSaveToast('Report not saved. Check update policy (RLS) for MDCAT table.', 'error')
      return
    }
...
/* ── FETCH FROM "MDCAT" TABLE ────────────────────────── */
async function fetchQuestionsForPaper(paperId) {
  const response = await fetch(
    `${SUPABASE_URL}/rest/v1/MDCAT?test_id=eq.${encodeURIComponent(paperId)}&order=id.asc&select=*`,
...
async function isAnyQuestionVisible() {
  const response = await fetch(
    `${SUPABASE_URL}/rest/v1/MDCAT?limit=1&select=id`,
    { headers: await getHeaders() }
  )
...
async function loadQuestions() {
  ...
  try {
    ...
    if (!questions.length) {
      ...
      if (!anyVisible) {
        throw new Error(
          'No rows visible in the "MDCAT" table. If RLS is enabled, add a SELECT policy for authenticated users.'
        )
      }
      throw new Error(
        `No questions found for paper "${paperId}". Check that test_id values in the "MDCAT" table match this URL parameter.`
      )
    }
