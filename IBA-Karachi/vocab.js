;(async function () {
  ...
  if (error || !cards?.length) {
    ...
    document.getElementById('errorMsg').textContent =
      error?.message || 'No vocabulary cards found.'
    return
  }
  ...
  function setFilter(filter) {
    ...
    const labels = { all: 'all cards', mastered: 'mastered cards', 'in-progress': 'in progress cards', new: 'not started cards' }
  ...
  async function markCard(status) {
    ...
    setTimeout(() => { if (deckIndex < deck.length - 1) nextCard() }, 300)
  ...
  function updateStats() {
    ...
    setTimeout(() => { document.getElementById('masteryBar').style.width = pct + '%' }, 100)
  }
  ...
  function buildWordList() {
    ...
    allCards.forEach(card => {
      const status = progress[card.id] || 'new'
      const dotCls = status === 'mastered' ? 'dot-mastered' : status === 'in-progress' ? 'dot-in-progress' : 'dot-new'
