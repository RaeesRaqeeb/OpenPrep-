...
async function submitQuestionReport() {
  ...
  try {
    ...
    if (error) {
      if (error.code === '42703') {
        showSaveToast('Report setup missing: flag column not found in test table.', 'error')
      } else {
    ...
    if (!data?.id) {
      showSaveToast('Report not saved. Check update policy (RLS) for test table.', 'error')
      return
    }
